...
@Component({
  selector: 'app-cadastra-pagina-parceiro',
  standalone: true,
  ...
  currentStep = 0;
   ...
   planos: any[] = [];
    ...
    this.musicsPreview = this.musicsPreview.filter((track) => track.id !== music.id)
     ...
     this.form = {
      ...this.form,
      playlist: this.musicsPreview.map(track => track.id), 
    };
   ...
   ngOnInit(): void {
    const sessionId = localStorage.getItem('sessionId');
    const dadosSalvos = localStorage.getItem('dadosCadastro_' + sessionId);
    const idParceiro = localStorage.getItem('uuidParceiro');
   ...
   setCriarNova(){
    ...
    async goToCheckout(){
       this.http.get<any>(`${apiUrl}/parceiros/getParceiro/${this.parceiroId}`).subscribe((res) => {      
          ...
          valor: res.precoCompra,
          planoSelecionado: 'Acesso-vitalicio',
          album: []
        } ;
          console.log("olja aqui ooo", formStorage);
   ...
   async removeImage(index: number): Promise<void> {
    ...
    prevStep() {
     ...
     loginWithSpotify(): void {
      ...
      localStorage.setItem('dadosCadastro_' + sessionId, JSON.stringify(formStorage));
      setTimeout(() => {
      window.location.href = `http://accounts.spotify.com/authorize?response_type=code&client_id=${this.clientId}&scope=app-remote-control streaming&redirect_uri=https://lovelink-frontenddeploy.vercel.app/cadastraPaginaParceiro`;
    }, 300)
    }
